...
def procesar_datos_de_distancia(df_resultados, n_clusters=3):
    ...
    for i, row in df.iterrows():
        ...
        for j, dist in enumerate(dist_list):
            ...
            # El índice de la ruta como identificador del vehículo
            vehiculos.append(j)
    ...
    df_distancias_ordenado = df_distancias.sort_values(
        by='distancia').reset_index(drop=True)
    ...
    df_distancias_ordenado['cluster_ordenado'] = df_distancias_ordenado['cluster'].map(
        label_map)
...
def resolver_precio_optimo_inv(df_distancias_ordenado, df_stock, df_demanda, df_zonas, beta=0.0152, theta=0.9, max_price=55.9, num_precios=100, P_LB=7, P_UB=46, M=1900000):
    ...
    df_distancias_ordenado['stock_actual'] = df_distancias_ordenado['tienda'].map(
        df_stock.set_index('id_tienda')['stock_actual'])
    # Unimos df_zonas con df_demanda para agregar la tienda correspondiente a cada zona
    df_merged = pd.merge(
        df_demanda, df_zonas[['id_zona', 'tienda_zona']], on='id_zona', how='left')

    # Agrupar por 'tienda_zona' y sumar la 'venta_digital' para obtener la demanda total por tienda
    df_demanda_por_tienda = df_merged.groupby(
        'tienda_zona')['venta_digital'].sum().reset_index()

    # Calcular I_UB como el 1.3% de la demanda total por tienda
    # 1.3% de la demanda total por tienda
    df_demanda_por_tienda['I_UB'] = df_demanda_por_tienda['venta_digital'] * 1.3
    ...
    for k, row in df_distancias_ordenado.iterrows():
        ...
        I_UB_tienda = df_demanda_por_tienda.loc[df_demanda_por_tienda['tienda_zona']
                                                == tienda, 'I_UB'].values[0]

        if stock_actual < I_UB_tienda:
            # Añadir las restricciones si el nivel de inventario es bajo
            # Ajuste según el número de clientes en el cluster (basado en simulación)
            if row['n_k (total clientes)'] > 500:
                W_i = 0
...
def zona_vehiculo(data_resultados, df_zonas):
    ...
    for _, zona_row in df_zonas.iterrows():
        ...
        tienda_row = data_resultados[data_resultados['tienda']
                                     == tienda_zona].iloc[0]
...
def resolver_precio_optimo_zona(matriz_ck, df_zona_vehiculo, df_stock, df_demanda, df_zonas, dia, beta=0.0152, theta=0.9, max_price=55.9, num_precios=100, P_LB=7, P_UB=46, M=1900000):
    ...
    df_resumen = pd.merge(df_resumen, df_zona_vehiculo[[
                          'id_zona', 'tienda_zona', 'vehiculo']], on='id_zona', how='left')
    df_resumen = df_resumen.drop(columns=['tienda_zona'])
    ...
    for k, row in df_final.iterrows():
        ...
        if stock_actual < I_UB_tienda_producto:
            # Añadir las restricciones si el nivel de inventario es bajo
            # Ajuste según el número de clientes en el cluster
            if row['n_k (total clientes)'] > limite_clientes:
                W_i = 0
                Z_i = 1  # Precio ajustado a P_UB
            else:
                # Si la demanda es baja, ajustamos el precio a P_LB
    ...
    df_precios_tienda_zona = df_precios_tienda_zona[[
        'id_zona', 'distancia', 'p_k (entero óptimo)', 'U_k (entero óptimo)', 'dia']]

    # Guardar df_precios_tienda_zona en un archivo CSV

    # Se hace afuera de la funcion, lo comento (Dante)
...
def caso_base_1(matriz_ck_base, df_zonas, dia):
    ...
    df_base['U_k'] = df_base['n_k (total clientes)'] * \
        df_base['A_k(p_k)'] * (df_base['p_k'] - df_base['centroide'])

    df_zonas = df_zonas.rename(columns={'tienda_zona': 'tienda'})
    df_base = pd.merge(
        df_base, df_zonas[['id_zona', 'tienda']], on='tienda', how='left')
    ...
    df_final.rename(columns={'p_k': 'p_k (entero óptimo)',
                    'U_k': 'U_k (entero óptimo)'}, inplace=True)
...
def caso_base_2(df_demanda, dia):
    ...
    """
    suma_por_zona = df_demanda.groupby(
        'id_zona')['venta_digital'].sum().reset_index()
    suma_por_zona = suma_por_zona.sort_values(
        by='venta_digital', ascending=False)
    ...
    df_tarifa_zona = suma_por_zona[['id_zona',
                                    'P_i', 'A_k(P_i)', 'U_k(P_i)']].copy()
    df_tarifa_zona.rename(columns={
                          'P_i': 'p_k (entero óptimo)', 'U_k(P_i)': 'U_k (entero óptimo)'}, inplace=True)
